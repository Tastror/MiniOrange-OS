...
// 定义目录项：占 32 个字节
typedef struct {
    BYTE  filename[8];       // 文件名：占 8 个字节
    BYTE  extension[3];      // 扩展名：占 3 个字节
    BYTE  proByte;           // 属性字节：占 1 个字节
    BYTE  sysReserved;       // 系统保留：占 1 个字节
    BYTE  createMsecond;     // 创建时间的 10 毫秒位：占 1 个字节
    WORD  createTime;        // 文件创建时间：占 2 个字节
    WORD  createDate;        // 文件创建日期：占 2 个字节
    WORD  lastAccessDate;    // 文件最后访问日期：占 2 个字节
    WORD  highClusterNum;    // 文件的起始簇号的高 16 位：占 2 个字节
    WORD  lastModifiedTime;  // 文件的最近修改时间
    WORD  lastModifiedDate;  // 文件的最近修改日期
    WORD  lowClusterNum;     // 文件的起始簇号的低 16 位：占 2 个字节
    DWORD filelength;        // 文件的长度：占 4 个字节
} Record, *PRecord;
...
#define RD_SECT(dev, sect_nr, fsbuf) \
    rw_sector( \
        DEV_READ, dev, (sect_nr)*SECTOR_SIZE, \
...
#define WR_SECT(dev, sect_nr, fsbuf) \
    rw_sector( \
        DEV_WRITE, dev, (sect_nr)*SECTOR_SIZE, \
