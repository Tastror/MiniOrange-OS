...
static int do_rdwt(MESSAGE *fs_msg)
{
    ...
    if (imode == I_CHAR_SPECIAL) {
        ...
        for (i = rw_sect_min; i <= rw_sect_max; i += chunk) {
            ...
            if (fs_msg->type == READ) {
                memcpy((void *)va2la(src, buf + bytes_rw), (void *)va2la(proc2pid(p_proc_current), fsbuf + off), bytes);
            } else { /* WRITE */
                memcpy((void *)va2la(proc2pid(p_proc_current), fsbuf + off), (void *)va2la(src, buf + bytes_rw), bytes);

                rw_sector(DEV_WRITE,  // modified by mingxuan 2019-5-21
                          pin->i_dev, i * SECTOR_SIZE, chunk * SECTOR_SIZE, proc2pid(p_proc_current), fsbuf);
            }
            off = 0;
            bytes_rw += bytes;
...
static int do_unlink(MESSAGE *fs_msg)
{
    ...
    memcpy((void *)va2la(proc2pid(p_proc_current), pathname), (void *)va2la(src, fs_msg->PATHNAME), name_len);
    pathname[name_len] = 0;
