...
; “这个跳转指令强制使用刚刚初始化的结构”——<<OS:D&I 2nd>> P90.
csinit:
    xor    eax, eax
    mov    ax, SELECTOR_TSS
    ltr    ax
...
%macro  hwint_master  1
    ...
    ; If k_reenter isn't equal to 0, there is no switching to the irq-stack, 
...
%macro exception_no_errcode 2
    ...
    ; we use esi to help to fetch arguments of exception handler from the stack.
...
divide_error:
    exception_no_errcode    0, divide_error_handler
...
save_int:
    ...
    mov     dword [ebx + ESP_SAVE_INT], esp     ;save esp position in the kernel-stack of the process
    mov     dx, ss
    mov     ds, dx
    mov     es, dx
    mov     fs, dx      ; value of fs and gs in user process is different to that in kernel
...
save_syscall:
    ...
    mov     dword [edx + ESP_SAVE_SYSCALL], esp     ; save esp position in the kernel-stack of the process
    mov     dx, ss
    mov     ds, dx
    mov     es, dx
    mov     fs, dx                  ; value of fs and gs in user process is different to that in kernel
...
restart_int:
    ...
    ; save current process's context, invoke schedule(), and then
...
restart_syscall:
    mov     eax, [p_proc_current]
    mov     esp, [eax + ESP_SAVE_SYSCALL]    ; restore esp position  xw
