...
void ReadSector(BYTE *buf, DWORD sectorIndex)
{
    ...
    // RD_SECT_SCHED_FAT(buf, sectorIndex);	// deleted by mingxuan
    // 2020-10-27
    RD_SECT_SCHED_FAT(
        fat32_dev, buf, sectorIndex
...
void WriteSector(BYTE *buf, DWORD sectorIndex)
{
    ...
    // WR_SECT_SCHED_FAT(buf, sectorIndex);	// deleted by mingxuan
    // 2020-10-27
    WR_SECT_SCHED_FAT(
        fat32_dev, buf, sectorIndex
    );  // modified by mingxuan 2020-10-27
    ...
    do {
        ...
        if (curSectorIndex >= last) {
            ...
            if (nextClusterIndex == FAT_END) {
                ...
                off = Position_Of_RootDir + (curClusterIndex - 2) *
                                                Sectors_Per_Cluster *
                                                Bytes_Per_Sector;
            }
        } else {
...
void GetNextSector(
    PFile pfile, DWORD curSectorIndex, PDWORD nextSectorIndex,
    PUINT isLastSector
)
{
    DWORD temp = 0;
    ...
    off_in_cluster = (BYTE
    )(temp % Sectors_Per_Cluster);  // 此扇区是所在簇的的几个扇区，从零开始
...
STATE ReadRecord(
    DWORD parentCluster, PCHAR name, PRecord record, PDWORD sectorIndex,
...
STATE FindSpaceInDir(
    DWORD parentClusterIndex, PCHAR name, PDWORD sectorIndex,
    PDWORD off_in_sector
)
{
    PBYTE  buf = NULL;
...
void CreateRecord(
    PCHAR filename, BYTE type, DWORD startCluster, DWORD size, PRecord precord
)
{
    WORD time[2];
    CHAR name[256] = {0};
...
STATE AddCluster(
    DWORD startClusterIndex, DWORD num
)  // cluster表示该文件或目录所在目录的簇,num表示增加几个簇
{
    PDWORD clusters = NULL;
    DWORD  curClusterIndex = startClusterIndex, nextClusterIndex = 0;
