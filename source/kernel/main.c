...
static int initialize_processes()
{
    ...
    p_proc = proc_table;
    for (pid = 0; pid < NR_TASKS; pid++) {  // 1>对前NR_TASKS个PCB初始化,且状态为READY(生成的进程)
        /*************基本信息*********************************/
    ...
    for (; pid < NR_K_PCBS; pid++) {  // 2>对中NR_TASKS~NR_K_PCBS的PCB表初始化,状态为IDLE,没有初始化esp(并没有生成,所以没有代码入口,只是留位置)
        /*************基本信息*********************************/
    ...
    for (; pid < NR_PCBS; pid++) {  // 3>对后NR_K_PCBS~NR_PCBS的PCB表部分初始化,(名称,pid,stat,LDT选择子),状态为IDLE.
        /*************基本信息*********************************/
