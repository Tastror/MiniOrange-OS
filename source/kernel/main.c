...
static inline void LDT_init_kernel(PROCESS *p_proc, u16 selector_ldt)
{
...
static inline void LDT_init_user(PROCESS *p_proc, u16 selector_ldt)
{
    p_proc->task.ldt_sel = selector_ldt;
    memcpy(&p_proc->task.ldts[0], &gdt[SELECTOR_KERNEL_CS >> 3], sizeof(DESCRIPTOR));
...
static inline void register_init_kernel(PROCESS *p_proc)
{
...
static inline void register_init_user(PROCESS *p_proc)
{
    ...
    p_proc->task.regs.eflags = 0x0202;  // IF=1, 倒数第二位恒为 1
    return;
}
...
static int initialize_processes()
{
    ...
    // 1> 对前 NR_TASKS 个 PCB 初始化，且状态为 READY（生成的进程）
    // 这里是 0 号和 1 号进程
    for (pid = 0; pid < NR_TASKS; pid++) {
        ...
        // LDT
        LDT_init_kernel(p_proc, selector_ldt);

        // 寄存器初值
        register_init_kernel(p_proc);

        // 线性地址布局初始化
        // task 的代码数据大小及位置暂时是不会用到的，所以没有初始化
        ...
        p_proc->task.memmap.kernel_lin_limit = KernelLinBase + KernelSize;  // 内核大小初始化为 8M
        ...
        p_proc->task.regs.eip = (u32)p_task->initial_eip;  // 进程入口线性地址
        ...
        p_proc->task.esp_save_context = p_regs - 10 * 4;  // when the process is chosen to run for the first time, sched() will fetch value from esp_save_context
        eip_context = restart_restore;
        // initialize EIP in the context, so the process can start run. added by xw, 18/4/18
    ...
    // 2> 对中 NR_TASKS ~ NR_K_PCBS 的 PCB 表初始化，状态为 IDLE
    // 没有初始化 esp（并没有生成，所以没有代码入口，只是留位置）
    // 这里是 2 号和 3 号进程
    ...
    pid = NR_K_PCBS;
    {
        ...
        // 线性地址布局初始化
        // initial这些段的数据并不清楚，在变身init的时候才在exec中赋新值
        p_proc->task.memmap.text_lin_base = 0;
        p_proc->task.memmap.text_lin_limit = 0;
        p_proc->task.memmap.data_lin_base = 0;
        p_proc->task.memmap.data_lin_limit = 0;
        ...
        // some field about process switch
        // initialize esp_save_int, added by xw, 17/12/11
        p_proc->task.esp_save_int = p_regs;
        // p_proc->task.save_type = 1;
        // when the process is chosen to run for the first time, sched() will fetch value from esp_save_context
        p_proc->task.esp_save_context = p_regs - 10 * 4;
        eip_context = restart_restore;
        // initialize EIP in the context, so the process can start run. added by xw, 18/4/18
        *(u32 *)(p_regs - 4) = (u32)eip_context;
        // initialize EFLAGS in the context, IF=1, IOPL=1. xw, 18/4/20
        *(u32 *)(p_regs - 8) = 0x1202;
    ...
    for (pid = NR_K_PCBS + 1; pid < NR_PCBS; pid++) {
        ...
        // LDT
        LDT_init_user(p_proc, selector_ldt);

        // 寄存器初值
        register_init_user(p_proc);
